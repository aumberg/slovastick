...
(function() {
	if (window.slovastick && window.slovastick["off"]) {
		window.slovastick["off"]();
	}
	...
	*/
	...
	//
	var $ = s.$ = window.jQuery.noConflict(true);
	$.fn.reverse = [].reverse;
	/*
	...
	*/
	...
	if (!window.document.evaluate) {
		$.getScript("http://wicked-good-xpath.googlecode.com/files/wgxpath.install.js")
			.success(function() {
	...
	$(function() {
		...
		(function x() {
			...
			elements.each(function() {
				...
				if (isChanged || !element.attr("tabindex") && !isTabindexUpdated) {
					s["update tabindex"](elements);
					isTabindexUpdated = true;
				}
			});
		}());
	...
	s["library audio play speech"] = function(text, lang) {
		...
		// var x = window.document.createElement("audio")
		// x.volume = volume / 100;
		// x.pause();
		// x.src = "http://translate.google.com/translate_tts?ie=UTF-8&q=github&tl=ru"
		// x.play()
		// s.green("text", audio.src)
	}
	// developing
	...
	s["library text self"] = function(element) {
		...
		var result 	= element.val() || clone.text() || clone.attr("title");
		clone.remove();
	...
	s["console"] = function(xpath) {
		if (xpath) {
			...
			s["memory current elements"] = s["library find"](xpath);
		}
	...
	s["console add xpath"] = function(xpath) {
		var result = s["console"]() ? s["console"]() + " | " + xpath : xpath;

		return s["console"](result);
	}
	//
	s["off"] = function (ele) {
		$(window).add($("*", "body"))
		...
		if (s["option program debug mode"]) {
			...
			$("[name='kick']", panel)
				.on("click.slovastick", function() {
					var boo 		= $(this).data("show");
					var elements 	= $("*", panel).not(this);

					$(this).data("show", !boo);
					boo ? elements.show(500) : elements.hide(500);
				});
		...
		$(window)
			...
			.on("keydown.slovastick", function(event) {
				...
				if (13 === event.which) {
					...
					s["memory last element"].click();

					return;
				}
				// if not shift button pressed
				if (16 !== event.which) {
					return;
				}
				//
				$(window)
					.one("keydown.slovastick-shift", function(event) {
						$(window).off(".slovastick-shift");
					})
					.one("keyup.slovastick-shift", function(event) {
						var element = s["memory last element"];
						var current = s["memory current elements"];
						...
						if (current.filter(element).length === element.length) {
							...
							// if it input field start speech recognition
						...
						//
						s["update tabindex"](s["library find"](s["console"]()).add(element));
						element.data("slovastick", "").focus().mouseenter();	
						s["console add xpath"](s["library find"](element)["xpath"]);
					});
			})
