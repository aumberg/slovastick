...
(function() {
	...
	*/
	...
	s["library text self"] = function(element) {
		...
		var result 	= clone.text() || clone.val() || clone.attr("title");
		clone.remove();
	...
	s["off"] = function (ele) {
		$(window).add($("*", "body"))
		// $()
			.off(".slovastick .slovastick_memory_element");
	...
	s["on"] = function (ele) {
		...
		// 
		$("*", "body") //.not($("*", "#slovastick"))
			.on("click.slovastick_memory_element", function(event) {
				//maybe can recursive calling?
				s["memory last element"].focus();
			})
			.on("focus.slovastick_memory_element", function(event) {
			...
			.on("mouseenter.slovastick_memory_element", function(event) {
				event.preventDefault();
				event.stopPropagation();
